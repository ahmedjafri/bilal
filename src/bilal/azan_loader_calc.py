...
class CalcAzanLoader(AzanLoader):
    ...
    def __init__(self, config_loader: AzanConfigLoader):
        ...
        # Get timezone offset
        self.timezone = int(datetime.now(timezone.utc).astimezone().utcoffset().total_seconds() / 3600)
    ...
    def get_azans_for_day(self, date: datetime = datetime.now()) -> List[Azan]:
        ...
        times = self._get_pray_times().getTimes(
            ...
            (self.config_loader.getConfig().latitude, self.config_loader.getConfig().longitude),
            self.timezone
        )
